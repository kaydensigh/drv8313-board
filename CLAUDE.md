...
- **`kicad-skip` is installed** (v0.2.5; pure-Python s-expr editor, dep `sexpdata`; **no running KiCad needed**). Lives in the bundled Python's *user* site-packages (Program Files is read-only): `C:\Users\quite\OneDrive\Documents\KiCad\10.0\3rdparty\Python311\site-packages\skip`, so it imports from `& "C:\Program Files\KiCad\10.0\bin\python.exe"` as `import skip`. **It does full schematic surgery** (clone components, wires, labels, re-net), verified 2026-06-17 against this project — the source is the API reference (read `skip/eeschema/*.py`). Key calls:

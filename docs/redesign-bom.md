...
1. **C1 value** — 10 nF (TI's recommended CP flying cap) or 100 nF (what the board shipped with)? Both work; 10 nF matches the datasheet.
2. **R5 value** — BOM says 1 kΩ, the manufacturer part number decodes to 4.7 kΩ. Confirm from the physical board before reusing; then size for the 3.3 V rail.
3. **LED1 re-site** — confirm moving the indicator from VM to the 3.3 V rail (recommended).
